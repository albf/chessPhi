...
struct moviment * alpha_beta(int F[8][8], int max_depth, int player, double * score) {
    ...
    for(i=0; i<=max_depth; i++) {            // Alpha=-inf ; Beta = inf
        ...
        best_score[i].alpha = ALPHA_SHARED;        // -inf. 
        best_score[i].beta = 999999;
        mov_counter[i] = 0;
    }

    if(ALPHA_SHARED > -999999) {
        printf("Using alpha_shared\n");
    }
...
struct exec_queue{
    ...
    pthread_mutex_t ga_lock;
};

void insert_f_mov(struct exec_queue * Q, int f_index,  struct moviment * mov, int counter) {
    struct f_mov * n_mov = (struct f_mov *) malloc (sizeof(struct f_mov));

    n_mov->f_index = f_index;
    n_mov->mov = *mov;
    n_mov->next = Q->move_list;
    n_mov->counter = counter;
...
void init_exec_queue(struct exec_queue * Q) {
    ...
    pthread_mutex_init(&Q->ga_lock, NULL);
}
...
void update_f_move(struct exec_queue *Q, int f_index) {
    double min=999999;
    struct f_mov * mov = Q->move_list;
    struct res_entry * res = Q->result; 

    while((mov!=NULL) && (mov->f_index != f_index)) {
        mov=mov->next;   
    }

    if(mov!=NULL) {
        pthread_mutex_lock(&Q->ga_lock);
        //printf("Counter[f_index=%d] = %d. Mov->j_index : %d\n", f_index, mov->counter, mov->f_index);
        mov->counter--;
        if(mov->counter==0) {
            printf("f_index:%d finished\n", f_index);
            while(res!=NULL) {
                if(res->f_index == f_index) {
                    if(res->score < min) {
                        min = res->score;
                    }
                }
            ...
            if(ALPHA_SHARED < min) {
                printf("Alpha Shared updated to %f.\n", min);
                ALPHA_SHARED = min;
            }             

        }

        pthread_mutex_unlock(&Q->ga_lock);
    }
    else {
        printf("Didn't found entry update_f_move %d.\n", f_index);
    }
...
struct moviment * parallel_chess(int F[8][8],int max_depth,int player,double *score)
{
    ...
    while(1) {  
      ...
      //printf("adding f_mov. n_root %d, counter %d\n", n_root, counter);
            current_score += undo_move(F,P,&next_root,(max_depth+1));
            insert_f_mov(&exec_q, n_root, &next_root,counter);
...
void *Worker_Thread() {
    ...
    while(1) {
        ...
        best_move = alpha_beta(exec->F, exec->max_depth, exec->player, &score);
        if(best_move->refresh > 0) {
            push_res(&exec_q, exec->f_index, exec->s_index, score);
        }
        num_res++;
        update_f_move(&exec_q, exec->f_index);
